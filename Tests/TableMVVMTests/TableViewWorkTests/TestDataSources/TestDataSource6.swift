...
class TestDataSource7: XCTestCase {
    ...
    func testDataSourceIsEmpty() {
        ...
        dataSource.section1.cellsViewModels = []
        XCTAssertTrue(dataSource.isEmpty)
        dataSource.section6.cellsViewModels = [.init(string: "dog")]
        XCTAssertFalse(dataSource.isEmpty)
        dataSource.section6.cellsViewModels = []
    }
    ...
    func testTableDataSource1ReloadData() {
        let dataSource: SimpleDS7 = SimpleDS7.fallBack()
    ...
    // swiftlint:enable all
        var testMe: String?
        var colorMe: UIColor?
        let dataSource: SimpleDS7 = SimpleDS7.fallBack { textVM, _ in
    ...
    func testTableDataSource1cellForRowAt0() {
        let dataSource: SimpleDS7 = SimpleDS7.fallBack()
        let table = UITableMVVM<SimpleDS7>()
    ...
    func testOutsideBounds() {
        ...
        XCTAssertFalse(
            dataSource.tableView(table, cellForRowAt: .init(item: 0, section: 7)) is CellTF ||
                dataSource.tableView(table, cellForRowAt: .init(item: 0, section: 7)) is CellCV
        )
        XCTAssertEqual(dataSource.tableView(table, numberOfRowsInSection: 7), 0)
        XCTAssertEqual(dataSource.tableView(table, heightForHeaderInSection: 7), 0)
        XCTAssertNil(dataSource.tableView(table, viewForHeaderInSection: 7))
        // Not sure how to test `do {}`
        dataSource.tableView(table, didSelectRowAt: .init(row: 0, section: 7))
        XCTAssertEqual(dataSource.numberOfSections(in: table), 7)
    }
}
